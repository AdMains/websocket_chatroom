...
class ZPHP
{
    ...
    private static $projectPath;//2017.7.10
    ...
    public static function run($rootPath, $run = true, $configPath = null)
    {
        ...
        self::$projectPath = $rootPath;//2017.7.10
//        self::setRootPath($rootPath);//2017.7.11
        if (empty($configPath)) {
            if (!empty($_SERVER['HTTP_HOST'])) {
                $configPath = \str_replace(':', '_', $_SERVER['HTTP_HOST']);
                $rootPath = $rootPath.DS.'http';//2017.7.11
            } elseif (!empty($_SERVER['argv'][1])) {
                $configPath = $_SERVER['argv'][1];
                $rootPath = $rootPath.DS.$configPath;//2017.7.11
            }
        }
        self::setRootPath($rootPath);//2017.7.11
        if (!empty($configPath)) {
