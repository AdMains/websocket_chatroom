...
class Redis implements IConn
{
    ...
    //2017.7.15新增方法
    public function addChannelOwner($fromId)
    {
        $this->redis->sAdd('totalChannelOwner',$fromId);
    }

    public function addNewChanneltoOwner($fromId, $channelName)
    {
        return $this->redis->sAdd($this->getKey($fromId,'cf'),$channelName);
    }

    public function getChannelOwner()
    {
        return $this->redis->sMembers('totalChannelOwner');
    }

    public function getNewChannel($fromId)
    {
        $arr =  $this->redis->sMembers($this->getKey($fromId,'cf'));
        if(empty($arr)){
            $this->redis->sRem('totalChannelOwner',$fromId);
        }
        return $arr;
    }

    public function delNewChannel($fromId,$channel)
    {
        if(!$this->redis->sIsMember('totalChannelOwner',$fromId)){
            return false;
        }
        if(!$this->redis->sIsMember($this->getKey($fromId,'cf'),$channel)){
            return false;
        }
        $this->redis->sRem($this->getKey($fromId,'cf'),$channel);
        return true;
    }
}